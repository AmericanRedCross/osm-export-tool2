                ...
                $.each(data, function(index, run){
                    ...
                    $.each(tasks, function(i, task){
                        ...
                        switch (task.name) {
                            ...
                            case 'MBTiles Export':
                                if (status === 'SUCCESS') {
                                    $taskDiv.append('<tr><td><a href="' + result.url + '">' + gettext('MBTiles') + '</a></td><td>' + duration + '</td><td>' +
                                        result.size + '</td></tr>');
                                }
                                break;
                        }
            ...
            $.each(tasks, function(i, task){
                ...
                switch (task.name) {
                    ...
                    case 'MBTiles Export':
                        if (status === 'PENDING' || status === 'RUNNING' || status === 'FAILED') {
                            cls = status.toLowerCase();
                            $taskDiv.append('<tr class="' + cls + '" id="' + task.uid + '"><td>' + gettext('MBTiles') + '</td><td> -- </td><td> -- </td><td>' + task.status + '</td></tr>');
                        }
                        else {
                            cls = status.toLowerCase();
                            $taskDiv.append('<tr class="' + cls + '" id="' + task.uid +'"><td><a href="' + result.url + '">' + gettext('MBTiles') + '</a></td><td>' + duration + '</td><td>' +
                            result.size + '</td><td>' + task.status + '</td></tr>');
                        }
                        break;
                }
                ...
                switch (task.name) {
                    ...
                    case 'MBTiles Export':
                        if (status === 'PENDING' || status === 'RUNNING' || status === 'FAILED') {
                            ...
                            $tr.removeClass();
                            $tr.addClass(status.toLowerCase());
                            $tr.html('<td><a href="' + result.url + '">' + gettext('MBTiles') + '</a></td><td>' + duration + '</td><td>' +
                            result.size + '</td><td>' + task.status + '</td>');
                        }
                        break;
                }
